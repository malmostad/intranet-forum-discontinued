...
function onelogin_saml_consume(){
  if (drupal_session_started()){
    ...
    require_once DRUPAL_ROOT . '/sites/all/modules/contrib/onelogin_saml/settings.php';
    require_once DRUPAL_ROOT . '/sites/all/modules/contrib/onelogin_saml/lib/onelogin/saml.php';
...
function onelogin_saml_auth($samlresponse){
  $user = db_query("SELECT * FROM {users} WHERE name = :cn AND status = 1", array(':cn' => $samlresponse->get_username()))->fetchObject();
  if ($user){
